...
def group_group_collide(group1, group2):
    ...
    for item1 in set(group1):
        for item2 in set(group2):
            if item1.collide(item2):
                score += 10
                group1.remove(item1)
                group2.remove(item2)
                explosion_group.add(Sprite(item1.pos, [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound))
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            ...
            current_explosion_center = [explosion_info.get_center()[0] + current_explosion_index * explosion_info.get_size()[0],
                                        explosion_info.get_center()[1]]
            canvas.draw_image(explosion_image, current_explosion_center, explosion_info.get_size(), self.pos, explosion_info.get_size(), 0)
        else:
            canvas.draw_image(self.image, self.image_center, self.image_size,
                              self.pos, self.image_size, self.angle)
...
def rock_spawner():
    ...
    if len(rock_group) < 12:
        if dist(rock_pos, my_ship.pos) - (my_ship.radius + 45) > 0:
